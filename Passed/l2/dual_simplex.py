...
class DualSimplex(object):
    ...
    def solve(self):
        ...
        while self.steps:
            ...
            is_optimal = all([self.dh[0][ind] + EPS >= plan[ind] >= self.dl[0][ind] - EPS for ind in self.Jb])
            ...
            jk, k = np.inf, -1
            for ind, val in enumerate(self.Jb):
                if self.dh[0][val] < plan[val] + EPS or plan[val] < self.dl[0][val] - EPS:
                    if val < jk:
                        jk, k = val, ind
